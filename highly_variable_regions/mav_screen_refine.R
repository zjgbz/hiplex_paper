install_if_missing <- function(pkg) {
  if (!requireNamespace(pkg, quietly = TRUE)) {
    ...
    bioc_packages <- c("ComplexHeatmap", "rtracklayer", "plyranges")
...
packages <- c("arrow", "matrixStats", "mgcv", "ggplot2", "cowplot", "ggExtra", "ggpointdensity")
...
wgc_qnorm_MAV_stat_dir_filename = file.path(out_dir, wgc_qnorm_MAV_stat_filename)
...
fig_dir_filename = file.path(out_dir, fig_filename)
