# ==============================================================================
# Biclustering Peak Annotation and RNA-seq Integration Analysis
# ==============================================================================
# This script performs comprehensive annotation of biclustered genomic regions
# using multiple annotation databases (CCRE, ChromHMM, RepeatMasker) and
# integrates with RNA-seq data to analyze gene expression patterns.
# ==============================================================================
...
# Load and Process Biclustering Data ===========================================

# Load biclustering results
biclustering_result <- read.table(
  biclustering_result_file,
  header = TRUE,
  sep = "\t"
)

# Filter out specific labels
biclustering_result <- biclustering_result[
  !biclustering_result$label %in% c("Epitope_Specific", "Background"),
]

# Parse genomic positions
...
# Calculate Overlap with Whole Genome Biclustering ============================
# wg_overlap_res <- findOverlaps(biclustering_gr, wg_biclustering_gr)
# wg_overlap_df <- data.frame(
#   query = biclustering_gr$label[queryHits(wg_overlap_res)],
...
# Perform annotations for each cluster
for (cluster_id in unique_clusters) {
  # Extract cluster-specific data
  biclustering_result_i <- biclustering_result[
    biclustering_result$label == cluster_id,
  ]
  
  biclustering_grange_i <- makeGRangesFromDataFrame(
  ...
  # Perform various annotations
  chipseeker_ccre_annotation <- annotatePeakByOverlappingChIPSeekerCCRE(
    biclustering_grange_i,
    annotation,
    categories
  )
  
  chipseeker_ccre_celltype_agnostic_annotation <- annotatePeakByOverlappingChIPSeekerCCRE(
    biclustering_grange_i,
    annotation_celltype_agnostic,
    categories,
    featureColname = "V6"
  )
  
  repeatmasker_annotation <- annotatepeakByOverlappingRepeatMasker(
    biclustering_grange_i,
    annotationRepeatMasker,
    repeatMaskerFeatures
  )
  
  chromhmm_short_annotation <- annotatepeakByOverlappingChromHMM(
    biclustering_grange_i,
    annotationChromHMM,
    categoriesChromHMM,
    featureColname = "V4"
  ...
  chromhmm_full_annotation <- annotatepeakByOverlappingChromHMM(
    biclustering_grange_i,
    annotationChromHMM,
    unique(annotationChromHMM$full_anno),
    featureColname = "full_anno"
  )
  
  chromhmm_group_annotation <- annotatepeakByOverlappingChromHMM(
    biclustering_grange_i,
    annotationChromHMM,
    unique(annotationChromHMM$group),
    featureColname = "group"
  )
...
# Create CCRE Annotation Plot (K562-specific) ==================================
anno_ccre <- lapply(chipseeker_ccre_annotations, getAnnoStatCCRE)
anno_ccre.df <- list_to_dataframe(anno_ccre)
anno_ccre.df$Feature <- factor(
  anno_ccre.df$Feature,
  levels = ChIPSeekerCCRECategoriesOrder
)

cluster_levels <- cluster_all_levels[
  cluster_all_levels %in% unique(biclustering_result$label)
]
anno_ccre.df$.id <- factor(anno_ccre.df$.id, levels = rev(cluster_levels))

categoryColumn <- ".id"
p_ccre <- plotAnnoBar.data.frame.one.target(
  anno_ccre.df,
  categoryColumn = categoryColumn,
  colorOption = 1,
  features = ChIPSeekerCCRECategoriesOrder
) +
  theme(
  ...
  ggtitle("Cis-regulatory Elements K562-specific") +
  guides(fill = guide_legend(title = NULL, nrow = 5, reverse = TRUE))

# Create CCRE Annotation Plot (Cell-type Agnostic) ============================
...
p_ccre_agnostic <- plotAnnoBar.data.frame.one.target(
  anno_ccre.df,
  categoryColumn = categoryColumn,
  colorOption = 1,
  features = ChIPSeekerCCRECategoriesOrder
...
anno_repeatmasker.df$Feature <- factor(
  anno_repeatmasker.df$Feature,
  levels = c(repeatMaskerFeatures, "other")
)
anno_repeatmasker.df$.id <- factor(
  anno_repeatmasker.df$.id,
  levels = rev(cluster_levels)
)

p_repeat <- plotAnnoBar.data.frame.one.target(
  anno_repeatmasker.df,
  categoryColumn = categoryColumn,
  colorOption = 4,
  features = repeatMaskerCategoriesOrder
) +
  theme(
    legend.position = "bottom",
    plot.title = element_text(size = 20),
...
# Create ChromHMM Annotation Plots =============================================
...
anno_chromhmm_short.df$Feature <- factor(
  anno_chromhmm_short.df$Feature,
  levels = c(unique(annotationChromHMM$V4), "other")
)
anno_chromhmm_short.df$.id <- factor(
  anno_chromhmm_short.df$.id,
  levels = rev(cluster_levels)
)

p_chromhmm_short <- plotAnnoBar.data.frame.one.target(
  anno_chromhmm_short.df,
  categoryColumn = categoryColumn,
  colorOption = 6,
  features = c(unique(annotationChromHMM$V4), "other")
) +
  theme(
    legend.position = "bottom",
    plot.title = element_text(size = 20),
...
anno_chromhmm_full.df$Feature <- factor(
  anno_chromhmm_full.df$Feature,
  levels = c(sort(unique(annotationChromHMM$full_anno)), "other")
)
anno_chromhmm_full.df$.id <- factor(
  anno_chromhmm_full.df$.id,
  levels = rev(cluster_levels)
)

p_chromhmm_full <- plotAnnoBar.data.frame.one.target(
  anno_chromhmm_full.df,
  categoryColumn = categoryColumn,
  colorOption = 5,
  features = c(unique(annotationChromHMM$full_anno), "other")
) +
  theme(
    legend.position = "bottom",
    plot.title = element_text(size = 20),
  ...
  ggtitle("ChromHMM Elements (full ver.)") +
  guides(fill = guide_legend(title = NULL, nrow = 7, reverse = TRUE))
...
anno_chromhmm_group.df$Feature <- factor(
  anno_chromhmm_group.df$Feature,
  levels = c(unique(annotationChromHMM$group), "other")
)
anno_chromhmm_group.df$.id <- factor(
  anno_chromhmm_group.df$.id,
  levels = rev(cluster_levels)
)

p_chromhmm_group <- plotAnnoBar.data.frame.one.target(
  anno_chromhmm_group.df,
  categoryColumn = categoryColumn,
  colorOption = 4,
  features = c(unique(annotationChromHMM$group), "other")
) +
  theme(
    legend.position = "bottom",
    plot.title = element_text(size = 20),
  ...
  ggtitle("ChromHMM Elements (group ver.)") +
  guides(fill = guide_legend(title = NULL, nrow = 5, reverse = TRUE))
...
# Calculate average expression for treatment and control
rnaseq_raw[, "V"] <- (rnaseq_raw[, "veh.1.RNA.seq_S1"] + 
                       rnaseq_raw[, "veh.2.RNA.seq_S2"]) / 2
rnaseq_raw[, "T"] <- (rnaseq_raw[, "NaB.1.RNA.seq_S3"] + 
                       rnaseq_raw[, "NaB.2.RNA.seq_S4"]) / 2

# Select relevant columns and transform
...
rnaseq_diff <- rnaseq_diff[abs(rnaseq_diff$log2FoldChange) >= 1, ]
rnaseq_diff <- rnaseq_diff[abs(rnaseq_diff$padj) <= 0.5, ]

# Find Overlapping Promoters and Genes ========================================
...
for (cluster_id in unique_clusters) {
  ...
  # Convert peaks to centers
  biclustering_result_i[, "start"] <- round(
    (as.numeric(biclustering_result_i[, "end"]) - 
     as.numeric(biclustering_result_i[, "start"])) %/% 2
  ) + as.numeric(biclustering_result_i[, "start"])
  ...
  overlap_res <- distanceToNearest(biclustering_grange_i, promoter_grange)
  ...
  cluster_ids <- append(
    cluster_ids,
    rep(glue("{cluster_id}"), length(overlaped_promoters))
  )
...
if (is_grouped_df_plot == TRUE) {
  ...
  for (fold_change in unique(biclustering_result$fold_change)) {
    for (cluster_id in unique_clusters) {
      ...
      biclustering_result_i <- biclustering_result_i[
        biclustering_result_i$fold_change == fold_change,
      ]
      ...
      overlaped_promoters <- promoter_grange[unique(subjectHits(overlap_res))]
      overlapped_genes <- overlaped_promoters$gene_id[
        overlaped_promoters$gene_id %in% rownames(rnaseq_diff)
      ]
      
      log2fc_vals <- append(
        log2fc_vals,
        rnaseq_diff[overlapped_genes, "log2FoldChange"]
      )
      log2fc_cluster_ids <- append(
        log2fc_cluster_ids,
        rep(glue("{cluster_id}"), length(rnaseq_diff[overlapped_genes, "log2FoldChange"]))
      )
      fold_changes <- append(
        fold_changes,
        rep(fold_change, length(rnaseq_diff[overlapped_genes, "log2FoldChange"]))
      )
    }
  }
  
  df_plt_data <- data.frame(
    cluster_ids = log2fc_cluster_ids,
    rnaseq_vals = log2fc_vals,
    fold_changes = fold_changes
  )
  df_plt_data$fold_changes <- factor(
    df_plt_data$fold_changes,
    levels = c("up", "down")
  )
  
  p_RNA_df <- ggplot(
    df_plt_data,
    aes(x = cluster_ids, y = rnaseq_vals, fill = fold_changes)
  ) +
    geom_boxplot(alpha = 0.5) +
    ggtitle("mRNA log2FC") +
    scale_x_discrete(
      "cluster_ids",
      breaks = factor(cluster_levels, levels = rev(cluster_levels)),
      drop = FALSE
    ) +
    theme(
      plot.title = element_text(size = 20),
  ...
  # Standard plot without grouping
  log2fc_vals <- c()
  log2fc_cluster_ids <- c()
  ...
  p_RNA_df <- ggplot(df_plt_data, aes(x = cluster_ids, y = rnaseq_vals)) +
    geom_boxplot(fill = "#6CB0D6", alpha = 0.5) +
    ggtitle("mRNA log2FC") +
    scale_x_discrete(
      "cluster_ids",
      breaks = factor(cluster_levels, levels = rev(cluster_levels)),
  ...
  pdf(
    gsub(".pdf", "_dif_rnaseq.pdf", save_dir, fixed = TRUE),
    width = 38 / 2.4 + 5,
    height = 5
  )
  print(p_RNA_df)
...
result_df <- data.frame(
  cluster_id1 = cluster_id1s,
  cluster_id2 = cluster_id2s,
  set1 = set1s,
  set2 = set2s,
  overlap = overlap_cts
)
...
write.csv(
  result_df,
  gsub(".pdf", "_overlap_counts.csv", save_dir, fixed = TRUE),
  row.names = FALSE,
  quote = FALSE
)
...
plt_data <- data.frame(
  ...
  rnaseq_vals = rnaseq_vals,
  rnaseq_vals_T = rnaseq_vals_T,
  gene_ids = gene_ids
)
...
p_gene_percentage <- ggplot(
  gene_type_mat,
  aes(x = factor(Var1), y = Freq, fill = Var2)
) +
...
plt_data_extra$cluster_ids <- factor(
  plt_data_extra$cluster_ids,
  levels = rev(cluster_levels)
)
plt_data$cluster_ids <- factor(plt_data$cluster_ids, levels = rev(cluster_levels))

# Create RNA Expression Plot ===================================================
showTV <- FALSE

if (showTV == TRUE) {
  # Show both treatment and vehicle
...
# Create Shared/Unique Gene Plot ===============================================
p_RNA_shared <- ggplot(
  plt_data_extra,
  aes(x = cluster_ids, y = rnaseq_vals, fill = shared)
) +
...
#' Extract legend from ggplot or grob object
#'
#' @param plot ggplot or grob object
#' @param legend Legend position identifier
...
# Create Combined Plots and Save ===============================================

# Main plot with K562-specific annotations
...
final_plot <- plot_grid(
  combined_plot,
  plot_grid(legend1, legend3, nrow = 1),
  ncol = 1,
  rel_heights = c(1, 0.2)
)
...
# Plot with cell-type agnostic annotations
combined_plot <- plot_grid(
...
pdf(
  gsub(".pdf", "_agnostic.pdf", save_dir, fixed = TRUE),
  width = 30 / 2.4 + 5,
  height = 70 / 2.4
)
print(final_plot)
...
# Save ChromHMM full version plot
legend5 <- get_legend2(p_chromhmm_full, "bottom")
pdf(
  gsub(".pdf", "_chromhmm_full.pdf", save_dir, fixed = TRUE),
  width = 38 / 2.4 + 5,
  height = 40 / 2.4
)
